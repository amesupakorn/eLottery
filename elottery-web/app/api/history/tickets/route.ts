...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1) ดึงสลากทั้งหมดของ user + ข้อมูลงวด
    ...
    // 2) ดึงผลรางวัลของรายการที่ได้ทั้งหมด
    const purchaseIds = purchases.map(p => p.id);
    const results = purchaseIds.length
      ? await prisma.drawResult.findMany({ where: { purchase_item_id: { in: purchaseIds } } })
      : [];
    ...
    for (const r of results) {
      if (r.purchase_item_id != null && !winByPurchaseId.has(r.purchase_item_id)) {
        winByPurchaseId.set(r.purchase_item_id, { prize_amount: r.prize_amount.toString() });
      }
    }

    // 3) map → payload สำหรับ UI เดิม (TicketItem[])
    const items = purchases.map((p) => {
      ...
      return {
        ...
        // schema ปัจจุบันไม่มีเวลาซื้อใน purchase → ใช้เวลางวดแทน
